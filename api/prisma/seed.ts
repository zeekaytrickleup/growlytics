import { PrismaClient, Provider, InsightStatus, Role } from '@prisma/client';
...
const SEGMENTS = [
  { name: 'VIP', color: 'var(--amber)' },
  { name: 'Loyal', color: 'var(--emerald)' },
  { name: 'Repeat Buyers', color: 'var(--primary-2)' },
  { name: 'One-Time', color: 'var(--blue)' },
  { name: 'Dormant', color: 'var(--mute)' },
  { name: 'Churn Risk', color: 'var(--red)' },
];
...
const NORTHWIND: Dataset = {
  ...
  kpis: {
    revenue: { cur: 81200, prev: 68814 },
    orders: { cur: 1942, prev: 1782 },
    profit: { cur: 29400, prev: 25789 },
    roas: { cur: 3.8, prev: 4.32 },
    aov: { cur: 41.8, prev: 40.6 },
    cr: { cur: 3.4, prev: 3.21 },
    ltv: { cur: 186, prev: 172 },
    returning: { cur: 42, prev: 41.2 },
  },
  revenueSeries: [
...
const AURORA: Dataset = {
  ...
  name: 'Aurora Labs',
  kpis: {
    revenue: { cur: 142500, prev: 121000 },
    orders: { cur: 3210, prev: 2980 },
    profit: { cur: 51200, prev: 43800 },
    roas: { cur: 5.1, prev: 4.6 },
    aov: { cur: 44.4, prev: 40.6 },
    cr: { cur: 4.1, prev: 3.8 },
    ltv: { cur: 210, prev: 198 },
    returning: { cur: 48, prev: 45 },
  },
  revenueSeries: [
    { d: 'Mon', rev: 15200, prev: 13100 }, { d: 'Tue', rev: 17100, prev: 15300 },
    { d: 'Wed', rev: 14600, prev: 14000 }, { d: 'Thu', rev: 20200, prev: 17200 },
    { d: 'Fri', rev: 24800, prev: 19100 }, { d: 'Sat', rev: 28900, prev: 22400 },
    { d: 'Sun', rev: 21700, prev: 19900 },
  ],
  traffic: [
    { name: 'Organic', value: 31 }, { name: 'Paid Ads', value: 34 },
    { name: 'Email', value: 21 }, { name: 'Social', value: 9 }, { name: 'Direct', value: 5 },
  ],
  products: [
...
async function seedWorkspace(d: Dataset) {
  ...
  // Idempotent: clear this workspace's data, then reinsert.
  await prisma.metricSnapshot.deleteMany({ where: { workspaceId: d.id } });
  await prisma.insight.deleteMany({ where: { workspaceId: d.id } });
  await prisma.product.deleteMany({ where: { workspaceId: d.id } });
  await prisma.customer.deleteMany({ where: { workspaceId: d.id } });
  await prisma.segment.deleteMany({ where: { workspaceId: d.id } });
  ...
  const trafficRows = d.traffic.map((t) => ({
    workspaceId: d.id, source: Provider.GA4, metric: 'traffic', dimension: t.name, value: t.value, ts: now,
  }));
  await prisma.metricSnapshot.createMany({ data: [...kpiRows, ...revRows, ...trafficRows] });

  await prisma.product.createMany({ data: d.products.map((p) => ({ ...p, workspaceId: d.id })) });
  for (const s of SEGMENTS) await prisma.segment.create({ data: { ...s, workspaceId: d.id } });

  return { products: d.products.length, metrics: kpiRows.length + revRows.length + trafficRows.length };
}

async function main() {
  const a = await seedWorkspace(NORTHWIND);
  const b = await seedWorkspace(AURORA);

  // Demo user with membership in both workspaces (drives the workspace switcher).
  ...
  console.log('Seeded workspaces:', { Northwind: a, Aurora: b, user: user.email });
}
